...
| 箇所 | 現状 | 修正案 |
|------|------|--------|
| ファイル名 | `READEME.md` | `README.md`（スペルミス） |
| §9 リスク#4 | 「スケジュール過後」 | 「スケジュール遅延」 |
| §2.2.2 GPU欄 | 「RTX 3050以上」 | RTX 3050は旧世代。「RTX 4050以上」or「専用GPU搭載」に変更を推奨 |
| §3.1 バリューコマース | 「Yahoo!ショッピング経由」 | 現在のサービス名称確認が必要（LINEヤフーへの移行の影響） |
...
> [!TIP]
> 初期フェーズではPC専門店や大手家電の楽天ショップ（ドスパラ、NTT-X Store、ビックカメラ等）の`shopCode`をホワイトリストとして登録し、品質の高い結果を返すことを推奨。
